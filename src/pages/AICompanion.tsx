...
export function AICompanionChatOnly() {
  const [messages, setMessages] = useState<Message[]>([]);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [profileName, setProfileName] = useState<string | null>(null);
  ...
  return (
    <div className="flex flex-col w-full h-[600px] max-h-[70vh]">
      <div className="flex-1 rounded-2xl bg-white/80 backdrop-blur-sm shadow-lg p-6 mb-0 overflow-y-auto">
        <AnimatePresence>
          {messages.length === 0 ? (
            <motion.div 
              className="flex flex-col items-center justify-center h-full gap-4 text-myanmar-maroon/50"
